...
class Attacks:
    ...
    def __init__(self, skip_attacks=[], only_attacks=[], skip_conditional_actions=True, console=None, credentials=None):

        # if console is None:
        #     from lib.util.console import console
        # self.console = console
        self.credntials = credentials
    ...
    def compute(self, max_iterations=5, max_search_depth=""):
        ...
        converged = False
        if(self.credntials is not None):
            db = Neo4j(username=self.credntials['username'], password=self.credntials['password'])
        else:
            db = Neo4j()
